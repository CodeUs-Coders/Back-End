...
public interface ChannelRepositroy extends JpaRepository<Channel, Long> {
    ...
    @Query(value = "UPDATE Channel m SET m.pw = :#{#channel.pw}, m.nickname = :#{#channel.nickname}, m.img = :#{#channel.img}, m.link = :#{#channel.link}, m.msg = :#{#channel.msg} WHERE m.id = :#{#channel.id}", nativeQuery = false)
